...
class PostgresClientTest(TestCase):
    ...
    def setUp(self):
        self.postgres_client = PostgresClient(dbname=os.getenv('DB_NAME'), user=os.getenv('DB_USER'),
                                              password=os.getenv('DB_PASSWORD'), host=os.getenv('DB_HOST'))

    def test_type_select_all_tables_from_db(self):
        """Тест возвращаемого типа данных функции select_all_tables_from_db."""

        self.assertEqual(type(self.postgres_client.select_all_tables_name_from_db()), list)
    ...
    def test_type_select_all_from_table(self):
        ...
        self.assertEqual(type(self.postgres_client.select_all_from_table(table_name='test_table')), list)
    ...
    def test_type_select_columns_from_table(self):
        ...
        # Тестируем тип данных при их наличии.
        self.assertEqual(type(self.postgres_client.select_columns_from_table('test_table', 'id', 'firstname')), list)
    ...
    def test_create_and_delete_table(self):
        ...
        self.assertEqual(len(self.postgres_client.select_all_tables_name_from_db()), tables_count + 1)
        ...
        self.assertEqual(len(self.postgres_client.select_all_tables_name_from_db()), tables_count)
    ...
    def test_insert_and_delete_in_table(self):
        ...
        self.assertEqual(len(self.postgres_client.select_all_from_table('test_table')), values_count + 1)

        # Удаляем тестовую строку из таблицы
        self.postgres_client.delete_value_in_table('test_table', "email='testtest@testmail.ru'")

        # Сравниваем, что количество элементов в таблице равно первоначальному
        self.assertEqual(len(self.postgres_client.select_all_from_table('test_table')), values_count)
